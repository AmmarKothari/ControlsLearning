...
%                 fprintf('already in state %s\n', name);
        ...
        %   measured wheel encoder tick counts are stored in prev_ticks.
            ...
            delta_right = m_per_tick * delta_right_ticks;
            delta_left  = m_per_tick * delta_left_ticks;
            
            delta_center = (delta_right + delta_left)/2;
            delta_theta  = (delta_right - delta_left)/L;
                        
            x_dt = delta_center * cos(theta);
            y_dt = delta_center * sin(theta);
            theta_dt = delta_theta;
