...
for i = 1:size(x,2)
    ...
    axis([-1,1,-1,1])
    hold on
    plot(sin(x(1,i)), -cos(x(1,i)), 'ro') %pendulum end
    plot([0, sin(x(1,i))], [0, -cos(x(1,i))], 'k-') %rod
    hold off
    pause(0.01)
end
...
%% Actual Dynamics
t = linspace(0, duration, 250);
f = @(t,x) simplePendulumDynamics(x, ppval(soln.spline.control, t), param);
[TOUT, YOUT] = ode45(f, t, [0;0]);
plot(TOUT, YOUT(:,1), 'ro-') % position
hold on
