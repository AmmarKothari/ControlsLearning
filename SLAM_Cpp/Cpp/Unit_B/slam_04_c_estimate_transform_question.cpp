...
int main(){
    ...
    std::ofstream landmarks_file("estimate_transform.txt");
    ...
    for (size_t i = 0; i<lego_log_file.scan_data.size(); i++){
        ...
        // add function to filter world cylinders by found pairs.
        ...
        if (trafo.valid){
            ...
            std::cout << "Transformed Cylinders Count: " << transformed_world_cylinders.size() << std::endl;
        }
